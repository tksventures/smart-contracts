const { height, waitUntilBlock } = require('./localNode');
const wvs = 10 ** 8;
...
describe('Timelock test suite', async function() {
    ...
    describe('Permissions and unlocked state', () => {
        it('should only allow owner to timelock or withdraw', async () => {
            ...
            const accessDeniedWithdrawal = invokeScript(withdrawTxParams, accounts.external);
    
            await expect(broadcast(accessDeniedWithdrawal)).to.be.rejectedWith(accessDeniedMessage)
    ...
    describe('Locked state and time management', async () => {
        ...
        it('should unlock after designated time', async () => {
            ...
            const { trace } = await broadcast(finalWithdrawal);
            await waitForTx(finalWithdrawal.id);

            const [ unlock ] = trace; 
            const { data } = unlock.result;
            const [ stateChanges ] = data;

            expect(stateChanges.key).to.equal('locked');
            expect(stateChanges.value).to.equal('false');
